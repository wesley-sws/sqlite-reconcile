...
from merge import log_merge, terminal_mergetool
...
def test_configured_editor_uses_git_like_precedence(monkeypatch):
    ...
    monkeypatch.setattr(terminal_mergetool, "_git_config_editor", lambda: "nano")

    assert terminal_mergetool._configured_editor() == "nano"

    monkeypatch.setenv("GIT_EDITOR", "code --wait")
    assert terminal_mergetool._configured_editor() == "code --wait"
...
def test_configured_editor_falls_back_to_terminal_editor(monkeypatch):
    ...
    monkeypatch.delenv("VISUAL", raising=False)
    monkeypatch.delenv("EDITOR", raising=False)
    monkeypatch.setattr(terminal_mergetool, "_git_config_editor", lambda: None)
    monkeypatch.setattr(terminal_mergetool.shutil, "which", lambda name: "/usr/bin/nano")

    assert terminal_mergetool._configured_editor() == "nano"

    monkeypatch.setattr(terminal_mergetool.shutil, "which", lambda name: None)
    assert terminal_mergetool._configured_editor() == "vi"
...
def test_edit_sql_inline_reads_prefilled_replacement(monkeypatch):
    ...
    assert terminal_mergetool._edit_sql_inline("L1", "old sql") == (
...
def test_edit_sql_inline_keeps_unchanged_prefilled_sql(monkeypatch):
    ...
    assert terminal_mergetool._edit_sql_inline("L1", "SELECT 1") is None


def test_prompt_replacement_reprints_statement(monkeypatch, capsys):
    ...
    replacement = terminal_mergetool._prompt_replacement(statement, {})

    assert replacement is statement
...
def test_prompt_pair_resolution_accepts_order_without_replacement_prompts(
    ...
    resolution = terminal_mergetool._prompt_pair_resolution(
        conflict,
        ours,
        theirs,
        table_columns,
...
def test_prompt_pair_resolution_edits_statement_before_order(monkeypatch):
    ...
    assert [statement.sql_text for statement in resolution] == [
...
def test_prompt_replay_warning_uses_prefilled_external_editor(monkeypatch):
    ...
    replacement, should_restart = terminal_mergetool._prompt_replay_warning(
        statement,
        table_columns,
...
def test_prompt_standalone_resolution_prints_statement_once(monkeypatch, capsys):
    ...
    terminal_mergetool._prompt_standalone_resolution(
        conflict,
        [],
        [statement],
        {},
    ...
    output = capsys.readouterr().out
    assert output.count("R4: INSERT INTO coupons") == 1
...
def test_branch_replay_safety_reruns_later_statements_after_edit(tmp_path, monkeypatch):
    ...
    table_columns, _, _ = log_merge.load_schema_metadata_from_db(base)
    statements = [
        log_merge.make_logged_statement(
            branch="ours",
            branch_index=0,
...
def test_branch_replay_safety_uses_stored_warning_before_recomputing(
    tmp_path,
    monkeypatch,
):
    base = tmp_path / "base.db"
...
def test_consume_frontier_counts_keeps_unaccepted_statements():
    ours = [
        log_merge.make_logged_statement(
            branch="ours",
            branch_index=index,
            ...
            committed_at="2026-01-01T00:00:00",
            sql_text=f"UPDATE t SET x = {index}",
        )
    ...
    theirs = [
        log_merge.make_logged_statement(
            ...
            committed_at="2026-01-01T00:00:00",
            sql_text=f"UPDATE t SET y = {index}",
        )
        for index in range(2)
    ]

    remaining_ours, remaining_theirs = terminal_mergetool._consume_frontier_counts(
        ours,
        theirs,
        frontier_ours_count=1,
        frontier_theirs_count=2,
    )

    assert remaining_ours == ours[1:]
    assert remaining_theirs == []
