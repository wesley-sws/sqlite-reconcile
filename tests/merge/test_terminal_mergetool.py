...
def test_pair_resolution_uses_transaction_labels_not_branch_aliases(
    monkeypatch,
    capsys,
):
    ours = txs([
    ...
    output = capsys.readouterr().out
    assert "A = local transaction" not in output
    assert "B = remote transaction" not in output
    assert "Unknown label A" in output
    assert resolution.action == "replace"
    assert resolution.ours is None
    assert resolution.theirs == theirs[0]
